...
def contact(request):
    if request.method=="POST":
        ...
        if len(name)<2 or len(email)<3 or len(phone)<10 or len(content)<5:
            messages.error(request, "please fill the form correctally")
        else:
...
def search(request):
    ...
    if allPosts.count() == 0:
        messages.warning(request, "No such result found, please refine your query")
...
def handleSignup(request):
    if request.method == 'POST':
        ...
        myuser.save()
        messages.success(request, "Your GullyBlog account has been successfully created")
        return redirect('home')
    else:
...
def handleLogin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, "Invalid Values, Please try again")
            return redirect('home')
